import { RESEND_API_KEY, APP_URL, formatEuro, formatDataPT, layoutEmail, botaoEmail, enviarEmailResend } from '../_shared/email.ts';
...
// Modelos de e-mail específicos deste fluxo (trial, pagamento). O envio em
// si -- RESEND_API_KEY, layout, formatação -- vive em ../_shared/email.ts,
// partilhado com send-welcome-email.
...
async function enviarEmailDeEvento(tipo, userId, payload, supabaseUrl, serviceRoleKey) {
  ...
  try {
    ...
    const resultado = await enviarEmailResend(utilizador.email, { subject, html });
    if (!resultado.ok && !resultado.skipped) console.error('[PTMANAGER] falha ao enviar e-mail', tipo, resultado.error);
  } catch (error) {
